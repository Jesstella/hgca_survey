...
chi2 = rc(path + 'chi2.csv')
G_mag = rc(path + 'absolute_g_mags.csv')
...
print('There are ' + str(len(chi2)) + ' chi2 values.')
print('There are ' + str(len(G_mag)) + ' G magnitudes.')
...
acc_err = list(accels['acceleration_err'])
header.append('accel_error')
...
gaia_parallax_err = list(bigt['gaia_parallex_err'])
...
G_abs_mag = list(G_mag['G_mag'])
header.append('G_mag')

G_mag_err = list(G_mag['G_mag_error'])
header.append('G_mag_err')
